...
def step(state: OptState, f, grad_f, c_eq, c_ineq, jac_c_eq, jac_c_ineq, dim: int, num_points: int, qp_tol:float, eq_tol:float, ineq_tol:float, convergence_tol:float) -> OptState:
    x, lambda_, mu_, sigma_prev, B, iter_num, converged, progress = state
    ...
    def maratos_correction(x, dx):
        x_next = x + dx
        c_eq_next = c_eq(x_next) 
        c_ineq_next = c_ineq(x_next)

        b_eq_corr = c_eq_next - jnp.squeeze(A_eq @ dx)
        b_ineq_corr = c_ineq_next - jnp.squeeze(A_ineq @ dx)

        qp_corr = solve_qp(B, g, A_eq, -b_eq_corr, A_ineq, -b_ineq_corr, qp_tol)
    ...
    def h_function(x):
        ...
        #return jnp.sum(c_eq(x)**2) + jnp.sum(jnp.exp(c_ineq(x)))
    ...
    def choose_sigma(x, dx, g, H, p):
        hes = dx.T @ H @ dx
        m = 1.0
        sigma = (jnp.dot(g, dx) + (m/2) * hes) / ((1-p) * h_func)
        return jnp.where(sigma_prev > sigma, sigma_prev, sigma + 1)  # some slack
    
    sigma = choose_sigma(x, dx, g, B, 0.5)

    merit_current = merit(x, sigma)


    merit_derivative = jnp.dot(g, dx) - sigma * h_func


    alpha_min = jnp.float32(1.0)
    dx = jnp.where(merit(x + alpha_min*dx, sigma) > merit_current + 0.1*alpha_min*merit_derivative, dx + maratos_correction(x, dx), dx)

    
    def line_search_step(carry):
        alpha, merit_current, loop_count = carry
        return (alpha*tau, merit_current, loop_count + jnp.array(1, dtype=jnp.int32))


    def cond_line_search(carry):
        alpha, merit_current, loop_count = carry
        return jnp.logical_and(merit(x + alpha*dx, sigma) > merit_current + 0.1*alpha*merit_derivative, loop_count < 15)
        
    
    alpha_final, merit_current, loop_count = jnp.where(merit_derivative < 0.0, 
    jnp.array(jax.lax.while_loop(
        cond_line_search,
        line_search_step,
        (alpha, merit_current, jnp.array(0, dtype=jnp.int32)))
    ), jnp.array((0.0, merit_current, 0.0)))


    x_next = x + dx * alpha_final
    lambda_next = lambda_ + alpha_final * (lambda_next - lambda_)
    mu_next = mu_ + alpha_final * (mu_next - mu_)
    ...
    converged = jnp.logical_or(jnp.logical_or(jnp.linalg.norm(dx * alpha_final) < convergence_tol, loop_count >= 15), 
                               jnp.logical_or(alpha_final == 0.0, jnp.logical_and(objective_improvement < 1e-4, feasibilty)))
    ...
    progress_next = jnp.array([
        ...
        merit_derivative,  # merit derivative
        loop_count,  # line search loop count
        sigma,
        sk@B@sk
    ])
...
def sqp_solve(
    ...
    initial_progress = jnp.array([
        ...
        0.0,  # initial merit value (set to 0)
        0.0,  # initial merit derivative (set to 0)
        0.0,   # initial line search loop count (set to 0)
        initial_sigma,
        0.0
    ])
...
def sqp_solve_batch(world_info: WorldInfo,
    ...
    return batched_solve(world_info, optim_info, robot_info, sdf, x0, initial_sigma, initial_hessian_scale, max_iter)
